...
let resolve = (to, base) => {
  ...
  // 这里举了个例子 如果 to 为 ?a=b baseuri 为 /users?b=c 则最终应该跳转到 /user?a=b
  if (toSegments[0] === "") {
  ...
  // `.` 开头的 代表当前路径或上一级路径
  let allSegments = baseSegments.concat(toSegments); // .././one   /a/b/c/d => ['a', 'b', 'c', 'd'. '..', '.', 'one']
  let segments = [];
  for (let i = 0, l = allSegments.length; i < l; i++) {
    let segment = allSegments[i];
    if (segment === "..") segments.pop(); // 如果是 `..` 就 pop 掉最后一级的路径，达到回到上一级的效果
    else if (segment !== ".") segments.push(segment); // 将非 . 的 push 进数组
  }
